...
class TicketLaunchView(discord.ui.View):
    ...
    @discord.ui.button(label="Ticket Openen", style=discord.ButtonStyle.primary, custom_id="persistent_ticket_create", emoji="🎫")
    async def create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if existing and guild.get_channel(existing[0]):
            return await interaction.followup.send("Je hebt al een openstaand ticket!", ephemeral=True)
        ...
        await ticket_channel.send(content=interaction.user.mention, embed=success_embed("Welkom! Een moderator helpt je zo snel mogelijk.", title="Ticket Aangemaakt"), view=TicketControlView())
        await interaction.followup.send(f"Ticket aangemaakt: {ticket_channel.mention}", ephemeral=True)

class TicketControlView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Sluit Ticket", style=discord.ButtonStyle.danger, custom_id="persistent_ticket_close", emoji="🔒")
...
class GiveawayJoinView(discord.ui.View):
    ...
    @discord.ui.button(label="Meedoen", style=discord.ButtonStyle.success, custom_id="persistent_giveaway_join", emoji="🎉")
    async def join_giveaway(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not giveaway or giveaway[0] == 1:
            return await interaction.response.send_message("Deze giveaway is afgelopen.", ephemeral=True)
        ...
        if existing:
            ...
            await interaction.response.send_message("Je doet niet meer mee.", ephemeral=True)
        else:
            await db.execute("INSERT INTO giveaway_participants (message_id, user_id) VALUES (?, ?)", (msg_id, interaction.user.id))
            await interaction.response.send_message("Je doet mee met de giveaway!", ephemeral=True)
...
class ModerationBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        try:
            ...
            print(f"Succesvol {len(synced)} slash commands gesynchroniseerd.")
        ...
        for r in records:
            ...
            if guild:
                try:
                    ...
                    await guild.unban(user, reason="Tempban verlopen.")
                except Exception:
                    pass
        ...
        for g in ended_g:
            ...
            if channel:
                try:
                    ...
                    if not participants:
                        await channel.send(f"Giveaway voor **{prize}** is afgelopen! Geen deelnemers.")
                    else:
                        ...
                        await channel.send(f"🎉 Gefeliciteerd {mentions}! Jullie winnen **{prize}**!")
                    
                    embed = msg.embeds[0]
                    embed.title = "🎉 Giveaway Afgelopen!"
...
async def on_message(message):
    ...
    # Blokkeer uitnodigingslinks voor niet-admins
    if not message.author.guild_permissions.administrator:
        if "discord.gg/" in message.content.lower() or "discord.com/invite" in message.content.lower():
            await message.delete()
            return await message.channel.send(f"{message.author.mention}, geen Discord invites plaatsen!", delete_after=5)

    await bot.process_commands(message)

# --- SLASH COMMANDS ---
@bot.tree.command(name="ban", description="Verbanning een gebruiker.")
@app_commands.checks.has_permissions(ban_members=True)
async def ban(interaction: discord.Interaction, member: discord.Member, reason: str = "Geen reden opgegeven"):
    await member.ban(reason=f"{reason} (Door {interaction.user})")
    await interaction.response.send_message(embed=success_embed(f"{member.mention} is verbannen. | Reden: {reason}"))

@bot.tree.command(name="kick", description="Kick een gebruiker.")
@app_commands.checks.has_permissions(kick_members=True)
async def kick(interaction: discord.Interaction, member: discord.Member, reason: str = "Geen reden opgegeven"):
    await member.kick(reason=f"{reason} (Door {interaction.user})")
    await interaction.response.send_message(embed=success_embed(f"{member.mention} is gekickt. | Reden: {reason}"))

@bot.tree.command(name="timeout", description="Geef een gebruiker een timeout (in minuten).")
@app_commands.checks.has_permissions(moderate_members=True)
async def timeout(interaction: discord.Interaction, member: discord.Member, minutes: int, reason: str = "Geen reden"):
    duration = datetime.timedelta(minutes=minutes)
    await member.timeout(duration, reason=reason)
    await interaction.response.send_message(embed=success_embed(f"{member.mention} heeft een timeout van {minutes} minuten gekregen."))

@bot.tree.command(name="warn", description="Geef een officiële waarschuwing.")
...
async def warn(interaction: discord.Interaction, member: discord.Member, reason: str):
    ...
    await interaction.response.send_message(embed=warning_embed(f"{member.mention} gewaarschuwd! Reden: {reason}\nTotaal waarschuwingen: {count[0]}"))
...
async def purge(interaction: discord.Interaction, amount: int):
    ...
    await interaction.followup.send(embed=success_embed(f"{len(deleted)} berichten verwijderd."), ephemeral=True)

@bot.tree.command(name="ticketpanel", description="Stuur het ticket paneel naar dit kanaal.")
@app_commands.checks.has_permissions(administrator=True)
async def ticketpanel(interaction: discord.Interaction):
    embed = create_embed("Support Tickets", "Klik op de knop hieronder om een support ticket te openen.")
    await interaction.channel.send(embed=embed, view=TicketLaunchView())
    await interaction.response.send_message("Paneel verstuurd!", ephemeral=True)

@bot.tree.command(name="gstart", description="Start een giveaway.")
...
async def gstart(interaction: discord.Interaction, minutes: int, winners: int, prize: str):
    ...
    embed = create_embed("🎉 Giveaway!", f"**Prijs:** {prize}\n**Winnaars:** {winners}\n**Eindigt:** <t:{int(end_time.timestamp())}:R>")
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        raise ValueError("DISCORD_TOKEN ontbreekt in de environment variabelen!")
